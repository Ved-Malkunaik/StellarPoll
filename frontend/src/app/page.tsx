      ...
      if (connected) {
        try {
          ...
          if (addr) {
            ...
            // Check choice and balance
            const [choice, state] = await Promise.all([
              getVoterChoice(addr),
              getPollState()
            ]);
            setUserChoice(choice);
            if (state) {
              const balance = await getTokenBalance(addr, state.token);
  ...
  const handleVote = async (index: number) => {
    ...
    try {
      ...
      if (result && result.status !== "ERROR") {
        ...
        // Refresh choice and balance from contract to be sure
        const [newChoice, state] = await Promise.all([
          getVoterChoice(walletAddress),
          getPollState()
        ]);
        setUserChoice(newChoice);
        if (state) {
          const balance = await getTokenBalance(walletAddress, state.token);
          setUserTokenBalance(balance);
        }
